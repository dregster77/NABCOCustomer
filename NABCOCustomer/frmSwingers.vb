...
Public Class frmSwingers

    Private LoadingSpecs As Boolean
    Private SkipPop As Boolean

    Private Sub frmSwingers_Load(sender As Object, e As EventArgs) Handles Me.Load
        PopModelList()
        PopBtnImages()
        PopApplications()
        PopSwingersImages()
    ...
    Private Sub PopDoorSpecs()
        ...
        End Try
        chkMatchW.CheckState = CheckState.Checked
    End Sub
    ...
    Private Sub ClearVisibleSpecs()
        ...
        Set_Visibility(False, lblPanic, chkPanic, lblRHReveal, txtRHReveal, lblMatchW, chkMatchW, lblCenterJamb, txtCenterJamb, lblJambtoPivot,
                        txtJambtoPivot, lblRHOpMan, ddlRHOp, txtRHDoorW, ddlLHOp, lblLHOpMan, lblRHDoorWidth)
    End Sub

    ' Determines what information should be displayed when they are on the specs screen
    Private Sub GetDoorSpecs()
        If pvpApplication.Enabled = False Then Return
        If pvpSpecs.Enabled = False Then Return
        ...
        If DispPanic() Then
            Set_Visibility(True, lblPanic, chkPanic)
        End If
        ...
        Select Case ddlapplication.SelectedItem.Text
            Case "Single"
                ...
                Set_Visibility(False, lblRHReveal, txtRHReveal)
                Exit Select
            Case "Sim Pair"
                lblDoorWidth.Text = "LH Door Width:"
                Set_Visibility(True, lblMatchW, chkMatchW, lblJambtoPivot, txtJambtoPivot, lblRHDoorWidth, txtRHDoorW)
                Exit Select
            Case "Double Egress"
                lblDoorWidth.Text = "LH Door Width:"
                Set_Visibility(True, lblMatchW, chkMatchW, lblJambtoPivot, txtJambtoPivot, lblRHDoorWidth, txtRHDoorW, lblRHReveal, txtRHReveal)
                Exit Select
            Case "Dual"
                lblDoorWidth.Text = "LH Door Width:"
                Set_Visibility(True, lblMatchW, chkMatchW, lblJambtoPivot, txtJambtoPivot, lblRHDoorWidth, txtRHDoorW, lblRHReveal, txtRHReveal,
                               lblCenterJamb, txtCenterJamb)
                Exit Select
            Case "OpMan"
                lblDoorWidth.Text = "LH Door Width:"
                Set_Visibility(True, lblMatchW, chkMatchW, lblJambtoPivot, txtJambtoPivot, lblRHDoorWidth, txtRHDoorW, lblRHReveal, txtRHReveal,
                               lblCenterJamb, txtCenterJamb, lblLHOpMan, ddlLHOp, ddlRHOp, lblRHOpMan)
                Exit Select
        End Select
...
Endfun:
    ...
    Private Sub ddlapplication_SelectedIndexChanged(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.Data.PositionChangedEventArgs) Handles ddlapplication.SelectedIndexChanged
        ...
        Else
            ...
            If ddlapplication.SelectedItem.Text = "Dual" Or ddlapplication.SelectedItem.Text = "OpMan" Then
                ...
                Set_Visibility(True, lblSwing2, ddlSwing2, pctSwingTwo)
                PopSwingOptions(ddlSwing1, 0)
                PopSwingOptions(ddlSwing2, 1)
            Else
                PopSwingOptions(ddlSwing1, 2)
                Set_Visibility(False, lblSwing2, ddlSwing2, pctSwingTwo)
                lblSwing1.Text = "Swing:"
            End If
        End If

        If ShowSpecsTab() = True Then
            Set_Enabled(True, pvpSpecs, pvpAccessories, pvpReview)
        Else
            Set_Enabled(False, pvpSpecs, pvpAccessories, pvpReview)
        End If
    End Sub
    ...
    Private Sub ddlSwing1_SelectedIndexChanged(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.Data.PositionChangedEventArgs) Handles ddlSwing1.SelectedIndexChanged
        ...
        End If
        If ShowSpecsTab() = True Then
            pvpSpecs.Enabled = True
            pvpAccessories.Enabled = True
            pvpReview.Enabled = True
        Else
            pvpSpecs.Enabled = False
            pvpAccessories.Enabled = False
            pvpReview.Enabled = False
        End If
    ...
    Private Sub pvpSpecs_TabIndexChanged(sender As Object, e As EventArgs) Handles pvpSpecs.TabIndexChanged
        If pvMasterSwinger.SelectedPage Is pvpSpecs Then
            If ShowSpecsTab() = True Then
                If ddlSwing2.Visible = True Then
    ...
    Private Sub pvMasterSwinger_SelectedPageChanging(sender As Object, e As RadPageViewCancelEventArgs) Handles pvMasterSwinger.SelectedPageChanging
        ...
        If e.Page Is pvpAccessories Then
